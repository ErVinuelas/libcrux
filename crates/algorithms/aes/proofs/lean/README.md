...
Files are listed in dependency order: each one only uses files above it.

| Path | Contents |
| --- | --- |
| `extraction/libcrux_aes.lean` | The hax-generated model of the crate. Generated by `hax.py`, not checked in. |
| `Utilities.lean` | `get_elem` / `set_elem` and the other bit-sliced state accessors, plus the `rename_auto_n` tactic. |
| `cipher/gf8.lean` | GF(2^8) arithmetic: addition, `xtime`, multiplication. |
| `cipher/transpose_u8x16.lean` | Specification of the block load, and `transpose_correct`. |
| `cipher/sub_bytes.lean` | The FIPS-197 S-box table, the SubBytes specification, and the proofs about the S-box selector and one column. |
| `cipher/shift_rows.lean` | ShiftRows specification, and the proof for one bit-plane. |
| `cipher/mix_columns.lean` | MixColumns specification, the unrolled form of `mix_columns_state`, and the proof that the two agree. |
| `cipher/xor_key1.lean` | AddRoundKey specification. |
| `cipher/cipher_theorems.lean` | The correctness theorems for the round functions. |
| `key_expansion/key_expansion_step.lean` | Specification and proofs for `key_expand1` and `key_expansion_step`. |
| `key_expansion/aes_keygen_assist.lean` | SubWord, RotWord and Rcon specifications, and the `aes_keygen_assist` proofs. |
| `AxiomGuard.lean` | The `#assert_axioms` command that checks the trust base. |
| `LibcruxAes.lean` | Imports everything and checks the trust base of every theorem. |
| `lakefile.toml`, `lake-manifest.json`, `lean-toolchain` | Lake project, pinned hax Lean library, and Lean version. |
...
./hax.py prove --target lean       # runs `lake build` in proofs/lean
```
...
Most theorems below are Hoare triples about a hax-generated function, saying
that the function returns the value the specification prescribes. Three are
not: `get_elem_SBOX_correct` and `transpose_correct` are facts about the
specification that the other proofs use, and `mix_columns_state_eq_unrolled`
is an equation between two functions. None of them depend on `sorry`.
...
table, and over the two GF(2^8) lemmas that none of them uses. It fails the
build if any of them comes to rest on an axiom outside what this development
accepts, which is:
