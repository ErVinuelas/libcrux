...
pub(crate) struct params_and_key<'a> {
    pub fst: &'a [crate::hacl::hash_blake2b::blake2_params<'a>],
    pub snd: &'a [u8],
}
...
fn update_block(
    ...
    krml::unroll_for!(12, "i", 0u32, 1u32, {
        ...
        let s0: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(0u32) as usize];
        let s1: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(1u32) as usize];
        let s2: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(2u32) as usize];
        let s3: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(3u32) as usize];
        let s4: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(4u32) as usize];
        let s5: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(5u32) as usize];
        let s6: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(6u32) as usize];
        let s7: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(7u32) as usize];
        let s8: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(8u32) as usize];
        let s9: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(9u32) as usize];
        let s10: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(10u32) as usize];
        let s11: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(11u32) as usize];
        let s12: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(12u32) as usize];
        let s13: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(13u32) as usize];
        let s14: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(14u32) as usize];
        let s15: u32 = (&crate::hacl::impl_blake2_constants::sigmaTable)
            [start_idx.wrapping_add(15u32) as usize];
...
pub(crate) fn init(hash: &mut [u64], kk: u32, nn: u32) {
    ...
    let p: crate::hacl::hash_blake2b::blake2_params = crate::hacl::hash_blake2b::blake2_params {
        digest_length: 64u8,
        key_length: 0u8,
        fanout: 1u8,
        depth: 1u8,
    ...
    (&mut tmp)[0usize] = nn1 as u64
        ^ ((kk1 as u64).wrapping_shl(8u32)
            ^ ((p.fanout as u64).wrapping_shl(16u32)
                ^ ((p.depth as u64).wrapping_shl(24u32)
                    ^ (p.leaf_length as u64).wrapping_shl(32u32))));
...
fn update_key(wv: &mut [u64], hash: &mut [u64], kk: u32, k: &[u8], ll: u32) {
    ...
    if ll == 0u32 {
        crate::hacl::hash_blake2b::update_block(wv, hash, true, false, lb, &b)
    } else {
...
pub(crate) fn update_multi(
    ...
    for i in 0u32..nb {
        let totlen: fstar::uint128::uint128 = fstar::uint128::add_mod(
            prev,
        ...
        crate::hacl::hash_blake2b::update_block(wv, hash, false, false, totlen, b.1)
    }
}
...
fn update_blocks(
    ...
    let nb0: u32 = if rem == 0u32 && nb > 0u32 {
        nb.wrapping_sub(1u32)
    } else {
        nb
    };
    let rem0: u32 = if rem == 0u32 && nb > 0u32 {
        128u32
    } else {
        rem
    };
    crate::hacl::hash_blake2b::update_multi(len, wv, hash, prev, blocks, nb0);
    crate::hacl::hash_blake2b::update_last(len, wv, hash, false, prev, rem0, blocks)
}

#[inline]
fn update(wv: &mut [u64], hash: &mut [u64], kk: u32, k: &[u8], ll: u32, d: &[u8]) {
    let lb: fstar::uint128::uint128 = fstar::uint128::uint64_to_uint128(128u32 as u64);
    if kk > 0u32 {
        crate::hacl::hash_blake2b::update_key(wv, hash, kk, k, ll);
        if ll != 0u32 {
            crate::hacl::hash_blake2b::update_blocks(ll, wv, hash, lb, d)
        }
    } else {
        crate::hacl::hash_blake2b::update_blocks(
            ll,
            wv,
            hash,
...
#[derive(PartialEq, Clone)]
pub struct state_t {
    pub block_state: crate::hacl::hash_blake2b::block_state_t,
    pub buf: [u8; 128],
    pub total_len: u64,
}
...
pub(crate) fn malloc_raw<'a>(
...
) -> [crate::hacl::hash_blake2b::state_t; 1] {
    let mut buf: [u8; 128] = [0u8; 128usize];
    let wv: [u64; 16] = [0u64; 16usize];
    let b: [u64; 16] = [0u64; 16usize];
    let mut block_state: crate::hacl::hash_blake2b::block_state_t =
        crate::hacl::hash_blake2b::block_state_t {
            fst: kk.key_length,
    ...
    match block_state {
        crate::hacl::hash_blake2b::block_state_t { thd: last_node, .. } => {
            ...
            (&mut tmp)[2usize] = pv.node_depth as u64 ^ (pv.inner_length as u64).wrapping_shl(8u32);
            (&mut tmp)[3usize] = 0u64;
            let tmp0: u64 = (&tmp)[0usize];
            let tmp1: u64 = (&tmp)[1usize];
            let tmp2: u64 = (&tmp)[2usize];
    ...
    let s: crate::hacl::hash_blake2b::state_t = crate::hacl::hash_blake2b::state_t {
        block_state,
        buf,
        total_len: ite as u64,
    };
    let p0: [crate::hacl::hash_blake2b::state_t; 1] = [s];
    p0
}

fn index_of_state(s: &[crate::hacl::hash_blake2b::state_t]) -> crate::hacl::hash_blake2b::index {
    let block_state: &crate::hacl::hash_blake2b::block_state_t = &(s[0usize]).block_state;
    match *block_state {
        crate::hacl::hash_blake2b::block_state_t {
            fst: kk1,
            snd: nn,
...
fn reset_raw<'a>(
    state: &'a mut [crate::hacl::hash_blake2b::state_t],
    key: crate::hacl::hash_blake2b::params_and_key<'a>,
) {
    let block_state: &mut crate::hacl::hash_blake2b::block_state_t =
        &mut (state[0usize]).block_state;
    let buf: &mut [u8] = &mut (state[0usize]).buf;
    let i: crate::hacl::hash_blake2b::index = match *block_state {
        crate::hacl::hash_blake2b::block_state_t {
            fst: kk1,
            snd: nn,
            thd: last_node,
    ...
    match *block_state {
        crate::hacl::hash_blake2b::block_state_t { thd: last_node, .. } => {
            let i1: crate::hacl::hash_blake2b::index = crate::hacl::hash_blake2b::index {
                key_length: kk1,
                digest_length: nn,
                last_node,
            };
...
pub fn reset_with_key_and_params(
    s: &mut [crate::hacl::hash_blake2b::state_t],
    p: &[crate::hacl::hash_blake2b::blake2_params],
    k: &[u8],
) {
    let i1: crate::hacl::hash_blake2b::index = crate::hacl::hash_blake2b::index_of_state(s);
    lowstar::ignore::ignore::<crate::hacl::hash_blake2b::index>(i1);
    crate::hacl::hash_blake2b::reset_raw(
        s,
        crate::hacl::hash_blake2b::params_and_key { fst: p, snd: k },
    )
}
...
*/
pub fn reset_with_key(s: &mut [crate::hacl::hash_blake2b::state_t], k: &[u8]) {
    let idx: crate::hacl::hash_blake2b::index = crate::hacl::hash_blake2b::index_of_state(s);
    let salt: [u8; 16] = [0u8; 16usize];
    let personal: [u8; 16] = [0u8; 16usize];
    let p: crate::hacl::hash_blake2b::blake2_params = crate::hacl::hash_blake2b::blake2_params {
    ...
    crate::hacl::hash_blake2b::reset_raw(
        s,
        crate::hacl::hash_blake2b::params_and_key { fst: &p0, snd: k },
    )
}
...
behavior is unspecified if you violate this precondition.
*/
pub fn reset(s: &mut [crate::hacl::hash_blake2b::state_t]) {
    crate::hacl::hash_blake2b::reset_with_key(s, &[])
}
...
*/
pub fn update0(
    state: &mut [crate::hacl::hash_blake2b::state_t],
    chunk: &[u8],
    chunk_len: u32,
...
*/
pub fn digest(s: &[crate::hacl::hash_blake2b::state_t], dst: &mut [u8]) -> u8 {
    let block_state: &crate::hacl::hash_blake2b::block_state_t = &(s[0usize]).block_state;
    let i1: crate::hacl::hash_blake2b::index = match *block_state {
        crate::hacl::hash_blake2b::block_state_t {
            fst: kk,
            snd: nn,
            thd: last_node,
    ...
    let block_state0: &crate::hacl::hash_blake2b::block_state_t = &(s[0usize]).block_state;
    ...
    let wv: [u64; 16] = [0u64; 16usize];
    let b: [u64; 16] = [0u64; 16usize];
    let mut tmp_block_state: crate::hacl::hash_blake2b::block_state_t =
        crate::hacl::hash_blake2b::block_state_t {
            fst: i1.key_length,
    ...
    match *block_state0 {
        crate::hacl::hash_blake2b::block_state_t { f4: ref src_b, .. } => match tmp_block_state {
            crate::hacl::hash_blake2b::block_state_t {
                f4: ref mut dst_b, ..
    ...
    let ite: u32 = if r.wrapping_rem(128u32) == 0u32 && r > 0u32 {
        128u32
    } else {
    ...
    match tmp_block_state {
        crate::hacl::hash_blake2b::block_state_t { snd: nn, .. } => {
            crate::hacl::hash_blake2b::finish(nn as u32, dst, &tmp_block_state.f4)
        }
    };
    let block_state1: &crate::hacl::hash_blake2b::block_state_t = &(s[0usize]).block_state;
    match *block_state1 {
        crate::hacl::hash_blake2b::block_state_t {
            fst: kk,
            snd: nn,
...
pub fn info(s: &[crate::hacl::hash_blake2b::state_t]) -> crate::hacl::hash_blake2b::index {
    let block_state: &crate::hacl::hash_blake2b::block_state_t = &(s[0usize]).block_state;
    match *block_state {
        crate::hacl::hash_blake2b::block_state_t {
            fst: kk,
...
*/
pub fn copy(
    state: &[crate::hacl::hash_blake2b::state_t],
) -> [crate::hacl::hash_blake2b::state_t; 1] {
    let block_state0: &crate::hacl::hash_blake2b::block_state_t = &(state[0usize]).block_state;
    let buf0: &[u8] = &(state[0usize]).buf;
    let total_len0: u64 = (state[0usize]).total_len;
    let i: crate::hacl::hash_blake2b::index = match *block_state0 {
        crate::hacl::hash_blake2b::block_state_t {
            fst: kk1,
            snd: nn,
            thd: last_node,
    ...
    match *block_state0 {
        crate::hacl::hash_blake2b::block_state_t { f4: ref src_b, .. } => match block_state {
            crate::hacl::hash_blake2b::block_state_t {
                f4: ref mut dst_b, ..
            } => (dst_b[0usize..16usize]).copy_from_slice(&src_b[0usize..16usize]),
        },
    ...
    let p: [crate::hacl::hash_blake2b::state_t; 1] = [s];
    p
}
...
pub fn hash_with_key(
    ...
    crate::hacl::hash_blake2b::init(&mut b, key_len, output_len);
    crate::hacl::hash_blake2b::update(&mut b1, &mut b, key_len, key, input_len, input);
    crate::hacl::hash_blake2b::finish(output_len, output, &b);
...
*/
pub fn hash_with_key_and_params<'a>(
    output: &'a mut [u8],
    input: &'a [u8],
    input_len: u32,
    params: crate::hacl::hash_blake2b::blake2_params<'a>,
    key: &'a [u8],
) {
    let mut b: [u64; 16] = [0u64; 16usize];
    let mut b1: [u64; 16] = [0u64; 16usize];
    ...
    (&mut tmp)[0usize] = nn as u64
        ^ ((kk as u64).wrapping_shl(8u32)
            ^ ((params.fanout as u64).wrapping_shl(16u32)
                ^ ((params.depth as u64).wrapping_shl(24u32)
                    ^ (params.leaf_length as u64).wrapping_shl(32u32))));
    ...
    r2.0[2usize] = iv6·;
    r2.0[3usize] = iv7·;
    crate::hacl::hash_blake2b::update(
        &mut b1,
        &mut b,
    ...
    crate::hacl::hash_blake2b::finish(params.digest_length as u32, output, &b);
    lib::memzero0::memzero::<u64>(&mut b1, 16u32);
    lib::memzero0::memzero::<u64>(&mut b, 16u32)
}
