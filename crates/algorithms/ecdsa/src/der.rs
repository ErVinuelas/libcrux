...
/// length octet, and verifying that it is at most `MAX`.
///
...
/// INTEGERs.
fn read_len<const MAX: usize>(bytes: &[u8], pos: &mut usize) -> Option<usize> {
    const {
        assert!(MAX < 0x80, "short-form DER length cannot exceed 127");
    }
...
pub(crate) fn der_to_raw(der: &[u8]) -> Option<([u8; 32], [u8; 32])> {
    ...
    pos += 1;
    let seq_len = read_len::<{ MAX_DER_LEN - 2 }>(der, &mut pos)?;
...
mod tests {
    ...
    #[test]
    fn rejects_overly_long_integer() {
        // An INTEGER with length 34 (exceeding maximum 33 content bytes for P-256)
        let mut sig = [0u8; 2 + (2 + 34) + (2 + 1)];
        sig[0] = SEQUENCE_TAG;
        sig[1] = (2 + 34) + (2 + 1);
        sig[2] = INTEGER_TAG;
        sig[3] = 34;
    ...
    fn rejects_overly_long_sequence() {
        // A SEQUENCE with length exceeding MAX_DER_LEN - 2
        let mut sig = [0u8; MAX_DER_LEN + 1];
        sig[0] = SEQUENCE_TAG;
        sig[1] = (MAX_DER_LEN - 2 + 1) as u8;
        assert!(der_to_raw(&sig).is_none());
    }
}
